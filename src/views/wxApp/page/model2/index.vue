<template>
  <div class="page">
    <div class="flex items-center">
      <div class="flex w-[800px] flex-col" v-loading="loading">
        ...
        <div class="flex flex-wrap gap-2 p-2 ml-3">
          <div v-for="(item, index) in list" :key="index" class="flex items-center cursor-pointer">
            <img v-if="item.path" class="img2" :id="'img2' + index" :src="item.path" @click="selectImg(index)" />
            <img v-else class="img2" src="../loading.png" />
      ...
      <div>
        <img src="./model2.png" style="width: 300px; height: 150px" />
      </div>
    </div>
...
const add = () => {
  ...
  dialogTitle.value = '新增模块二图片'
  updateData.value = { path: '', name: '', type: 'model2' }
